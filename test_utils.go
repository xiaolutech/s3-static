...
func SetupTestSuite(t *testing.T) *TestSuite {
	...
	cfg := &config.Config{
		...
		CacheStrategy:        "no-cache", // Default strategy
		LogLevel:             "info",
	}
...
// SetupTestSuiteWithEnv creates a test environment with custom environment variables
func SetupTestSuiteWithEnv(t *testing.T, envVars map[string]string) *TestSuite {
	ctx := context.Background()
	...
	if cacheDuration, exists := envVars["CACHE_DURATION"]; exists {
		if duration, err := time.ParseDuration(cacheDuration); err == nil {
			cfg.DefaultCacheDuration = duration
		}
	}
	if logLevel, exists := envVars["LOG_LEVEL"]; exists {
